...
import { convertVnd, expandBets, printBetResultShakeDiskLive } from "../utils/betHelper.js"
import { logError, printTable } from "../utils/helperCmd.js"
import { CMD_BET, CMD_END, CMD_START } from "../contants/shake-disk-live.js"

const WebSocketClient = websocket.client
const __filename = fileURLToPath(import.meta.url)
const __dirname = dirname(__filename)
const configPath = path.resolve(__dirname, "../config/shake-disk-live.json")
...
function handleGameResultUpdate(parsedMessage, worker) {
  ...
  if (totalType) {
    const types = totalType.split(",").map(t => t.trim());
    worker.gameHistory.push(...types);
  
    // Giới hạn còn 20 phần tử cuối
    if (worker.gameHistory.length > 20) {
      worker.gameHistory.splice(0, worker.gameHistory.length - 20);
    }
  }
...
function executeBettingLogic(worker, gameData) {
  ...
  if (!bettingDecision.choices?.length) {
    ...
    printBetResultShakeDiskLive(bettingDecision)
  }
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.mainGameClient.on("connect", (connection) => {
        ...
        setTimeout(() => {
          this.mainGameConnection.sendUTF(`[6,"Livestream","ShakeDiskLivestreamPlugin",{"cmd":1950}]`)
          setTimeout(() => this.mainGameConnection.sendUTF(`[6,"Livestream","ShakeDiskLivestreamPlugin",{"cmd":1959}]`), 500)
        }, 200)

        resolve()
      })

      this.mainGameClient.connect("wss://ws-xocdia-ls.azhkthg1.com/websocket")
    })
  }
 ...
 */
export const startGameShakeDiskLive = async () => {
  if (activeGameWorker) {
  ...
  const categoryGame = "shake_disk_live";
 ...
 * Stop the game
 */
export const stopGameShakeDiskLive = () => {
  if (activeGameWorker) {
