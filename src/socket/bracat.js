...
import { convertVnd, expandBets, getLabelByValueBaracat, printBetResult, printBetResultBaracat } from "../utils/betHelper.js"
import { logError, printTable } from "../utils/helperCmd.js"
import { CMD_BET, CMD_BUDGET, CMD_END, CMD_START,CMD_JACKPOT, CMD_END_BET } from "../contants/bracat.js"

const WebSocketClient = websocket.client
const __filename = fileURLToPath(import.meta.url)
const __dirname = dirname(__filename)
const configPath = path.resolve(__dirname, "../config/bacarat.json")
...
function loadConfig() {
  try {
    ...
    if (JACKPOT_RANGES.length > 0) {
      console.log(chalk.yellow(`Khoảng Jackpot cho phép:`))
      JACKPOT_RANGES.forEach((range, idx) => {
        console.log(chalk.yellow(`  ${idx + 1}. ${convertVnd(range.MIN)} - ${convertVnd(range.MAX)}`))
      })
    }
...
// ============================================
// THAY ĐỔI 3: Thêm hàm isJackpotInAllowedRange() (sau sendBudgetWarning, dòng ~116)
// ============================================
/**
...
function handleMainGameMessage(msg, worker) {
  ...
  // Command 100: Budget update
   ...
   else if (messageString.includes(`"cmd":${CMD_END_BET}`)) {
    handleBetConfirmation(parsedMessage,worker)
  }
}
...
function handleGameResultUpdate(parsedMessage, worker) {
  ...
  const totalType = gameData?.betTypeResult
 ...
 * @param {GameWorker} worker 
 */
function handleBetConfirmation(parsedMessage, worker) {
  const gameData = parsedMessage[1]

  // Map danh sách cược (bs)
  const betDetails = gameData.bs
    .map(bet => `${chalk.yellow(bet.eid)}: ${chalk.red(bet.b.toLocaleString())}đ`)
    .join(", ")
  ...
  console.log(
    ...
    ` - Đặt cược: ${chalk.red(gameData.bs.reduce((sum, b) => sum + b.b, 0).toLocaleString())} đ. ` +
    chalk.magenta(`Chi tiết cược: `) + betDetails
  )
...
function handleJackpotUpdate(parsedMessage, worker) {
  ...
  if (data && typeof data.jackpotAmount === "number") {
    worker.currentJackpot = data?.jackpotAmount;
    console.log(
      chalk.blue(`[${getCurrentTime()}] `) +
      `Jackpot hiện tại: ${chalk.green(convertVnd(data?.jackpotAmount))} phiên #${chalk.yellow(data?.sessionId)}`
    );
...
function executeBettingLogic(worker, gameData) {
  ...
  if (!bettingDecision.choices?.length) {
    ...
    printBetResultBaracat(bettingDecision)
  }
  ...
  if (worker.mainGameConnection?.connected) {
    bets.forEach((bet, index) => {
      ...
      setTimeout(() => {
        const betCommand = `[6,"Livestream","BaccaratLivestreamPlugin",{"cmd":1952,"b":${bet.amount},"eid":"${bet.choice}","sid":${gameData?.sessionId}}]`
        ...
        worker.currentSessionId = gameData?.sessionId || null
      }, delay)
    })
  } else {
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.mainGameClient.on("connect", (connection) => {
        ...
        setTimeout(() => {
          this.mainGameConnection.sendUTF(`[6,"Livestream","BaccaratLivestreamPlugin",{"cmd":1950}]`)
          setTimeout(() => this.mainGameConnection.sendUTF(`[6,"Livestream","BaccaratLivestreamPlugin",{"cmd":1959}]`), 500)
        }, 200)

        resolve()
      })

      this.mainGameClient.connect("wss://livearena.azhkthg1.net/bcr")
    })
  }
 ...
 */
export const startGameBaracat = async () => {
  if (activeGameWorker) {
